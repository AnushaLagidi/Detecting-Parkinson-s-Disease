...
def load_split(path):
    ...
    # extract the class label from the filename
        ...
        # and resize it to 200x200 pixels, ignoring aspect ratio
        image = cv2.imread(imagePath)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        image = cv2.resize(image, (200, 200))
        
        # threshold the image such that the drawing appears as 
...
for i in range(0, args["trials"]):
    ...
    # make predictions on the testing data and 
    # initialize a dictionary to store our computed metrics
    predictions = model.predict(testX)
    metrics = {}

    # compute the confusion matrix and 
    # use it to derive the raw accuracy, sensitivity, and specificity
...
for i in idxs:
    ...
    # quantify the image and make predictions based on 
    # the extracted features using the last trained Random Forest
    features = quantify_image(image)
    preds = model.predict([features])
    label = le.inverse_transform(preds)[0]
    
    # draw the colored class label on the output image and 
    # add it to the set of output images
